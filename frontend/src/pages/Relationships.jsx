...
import { getUserRelations } from "../services/user.js";
import { getTransactionRelationship } from "../services/transaction.js";
...
const Relationships = () => {
  ...
  const makeNode = (id, label, type = "user") => ({
    data: { id, label, type },
  });
    ...
    relations.forEach((rel) => {
      ...
      if (relationship === "SHARED_ATTRIBUTE") {
        ...
        if (source.amount && source.ip) {
          sourceId =
            source.id || source.transaction_id || JSON.stringify(source);
          targetId = target.id || target.email || JSON.stringify(target);
          ...
          sourceId = source.id || source.email || JSON.stringify(source);
          targetId =
            target.id || target.transaction_id || JSON.stringify(target);
          sourceLabel = source.name || source.email || "User";
      ...
      if (!nodesMap.has(sourceId))
        nodesMap.set(sourceId, makeNode(sourceId, sourceLabel, sourceType));
      if (!nodesMap.has(targetId))
        nodesMap.set(targetId, makeNode(targetId, targetLabel, targetType));
